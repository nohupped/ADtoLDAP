...
func ConnectToADTLS(ADHost, AD_Port string, ADUsername, ADPassword string, ADConnTimeout int) (*ldap.Conn) {
	ldap.DefaultTimeout = time.Duration(ADConnTimeout) * time.Second
	Info.Println("Set AD connection timeout to", ldap.DefaultTimeout)
	Info.Println("Dialling TLS")
	l, err := ldap.DialTLS("tcp", fmt.Sprintf("%s:%s", ADHost, AD_Port), &tls.Config{InsecureSkipVerify: true})
	CheckForError(err)
	err = l.Bind(ADUsername, ADPassword)
	CheckForError(err)
	return l
